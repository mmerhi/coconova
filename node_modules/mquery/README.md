...
```js
var greatItems = mquery(itemCollection).where('rating').gte(4.5).toConstructor();

// use it!
greatItems().count(function (err, n) {
  console.log('There are %d great items', n);
});

greatItems().where({ name: /^Life/ }).select('name').find(function (err, docs) {
  console.log(docs);
});
```
